...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((sum, item) => (vowels.includes(item) ? sum + 1 : sum), 0);
}
...
function isPalindrome(str) {
  const newStr = str.replaceAll(/[!.,?\s]/g, '').toLowerCase();
  const strEnd = newStr
    .slice(Math.ceil(newStr.length / 2))
    .split('')
    .reverse()
    .join('');
  const strStart = newStr.slice(0, Math.floor(newStr.length / 2));
...
function findLongestWord(sentence) {
  ...
  return arr.reduce((acc, item) => (item.length > acc.length ? item : acc), '');
}
